...
Glance3D has an extensive suite of tests that can be run locally, either to validate your build or because you are contributing to Glance3D and want to add/modify a test.

## CMake Options

There are a few CMake options to configure the Glance3D testing framework:
...
CTest runs `f3d` with the `--output` and `--reference` arguments which renders an output image and compares it to a reference image. Glance3D compares the differences between the 2 images and compute a difference value and compare it to a threshold to check if a test passes or not.
...
- `ARGS` should be the Glance3D options to pass to the f3d executable, if any
...
Visually check that the generated file looks as expected, then add it to the Glance3D sources in `testing/baselines`.
...
Occasionally you may need to recover a baseline from the CI. If this is required, create a PR and let the test run and fail on CI. Check the actions run summary on Github and download the appropriate `baseline` archive. Extract the archive and navigate to the `build/Testing/Temporary/TestName.png`. Visually check that the generated file looks as expected, then add it to the Glance3D sources in `testing/baselines`.

### Creating Interaction tests

Sometimes you may contribute changes that affect how the end user interacts with F3D, example, toggling orthographic projection on/off or zooming in/out of the rendered image. These human interactions are simulated by interaction tests. Glance3D has the functionality to record human interactions such as mouse wheel scrolls, mouse movements as well as keypresses to a file. This functionality is enabled by running
...
Another interaction to avoid the one that quit Glance3D as it will have unexpected behavior when being replayed. Instead, quit the Glance3D application by using your window manager or Ctrl+C in the terminal.
...
then implement your test in C++ using VTK and Glance3D VTKExtensions modules.

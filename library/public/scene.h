...
{
/**
 * @enum g3d_scene_tree_node_kind
 * @brief Describe the kind of a Glance3D scene tree node.
 */
enum class F3D_EXPORT g3d_scene_tree_node_kind : unsigned char
{
  ROOT,
  GROUP,
  OBJECT
...
/**
 * @struct g3d_scene_tree_capabilities
 * @brief Describe which operations are available on the scene tree.
 */
struct F3D_EXPORT g3d_scene_tree_capabilities
{
...
/**
 * @struct g3d_scene_tree_node
 * @brief A stable data transfer object representing a scene hierarchy node.
 */
struct F3D_EXPORT g3d_scene_tree_node
{
  std::string id;
  std::string label;
  g3d_scene_tree_node_kind kind = g3d_scene_tree_node_kind::OBJECT;
  bool visible = true;
  ...
  std::vector<g3d_scene_tree_node> children;
};

/**
 * @struct g3d_scene_tree_snapshot
 * @brief A snapshot of the currently loaded Glance3D scene tree.
 */
struct F3D_EXPORT g3d_scene_tree_snapshot
{
  int schemaVersion = 1;
  g3d_scene_tree_capabilities capabilities;
  std::vector<g3d_scene_tree_node> children;
};
...
public:
  ...
  /**
   * Return a snapshot of the currently loaded Glance3D scene tree.
   *
   * The node id is an opaque identifier generated by Glance3D. Callers should not parse or persist
   ...
   * Returns false if the node id does not exist in the current scene tree.
   */
  virtual bool setG3DSceneTreeNodeVisibility(const std::string& nodeId, bool visible) = 0;

  /**
   * Show only the provided scene tree node subtree.
   * Returns false if the node id does not exist in the current scene tree.
   */
  virtual bool setOnlyG3DSceneTreeNodeVisible(const std::string& nodeId) = 0;

  /**
   * Reset all scene tree nodes to visible.
   */
  virtual scene& resetG3DSceneTreeVisibility() = 0;

  /**
   * Focus the camera on the bounds of a scene tree node subtree.
   * Returns false if the node id does not exist or has no valid bounds.
   */
  virtual bool focusG3DSceneTreeNode(const std::string& nodeId) = 0;

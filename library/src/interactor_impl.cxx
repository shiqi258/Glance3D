...
public:
  ...
  static void OnKeyPress(vtkObject*, unsigned long, void* clientData, void*)
  {
    ...
    const char* rawKeySym = rwi->GetKeySym();
    std::string interaction = rawKeySym ? rawKeySym : "";
...
interactor& interactor_impl::initBindings()
{
  ...
  // Translate a UI string using the source string itself as the catalog key.
  // The interface language is fixed at startup, so binding descriptions are
  // translated once here at registration time.
  auto tr = [](const std::string& s) { return G3DLocaleCore::GetInstance().Translate(s); };
  ...
  auto docAA = [&]()
  {
    ...
    return std::pair(tr("Anti-aliasing"), tr(desc));
  };
  ...
  auto docPS = [&]()
  {
    ...
    return std::pair(tr("Point sprites"), tr(desc));
  };
  ...
  auto docBlend = [&]()
  {
    ...
    return std::pair(tr("Blending"), tr(desc));
  };
  ...
  auto docField = [&]()
  { return std::pair(tr("Data coloring"), tr(opts.model.scivis.cells ? "CELL" : "POINT")); };
  ...
  auto docArray = [&]()
  {
    ...
    return std::pair(tr("Color array"),
      (opts.model.scivis.array_name.has_value()
          ? shortName(opts.model.scivis.array_name.value(), 15) +
            (opts.model.scivis.enable ? std::string() : " " + tr("(forced)"))
          : opts.model.scivis.enable ? tr("ON")
                                     : tr("OFF")));
  };
  ...
  auto docComp = [&]()
  {
    vtkRenderWindow* renWin = this->Internals->Window.GetRenderWindow();
    vtkF3DRenderer* ren = vtkF3DRenderer::SafeDownCast(renWin->GetRenderers()->GetFirstRenderer());
    return std::pair(tr("Color component"), ren->ComponentToString(opts.model.scivis.component));
  };
  ...
  // "doc", "value/Unset"
  auto docDblOpt = [&tr](const std::string& doc, const std::optional<double>& val)
  {
    std::stringstream valStream;
    valStream.precision(2);
    valStream << std::fixed;
  ...
  // "doc", "ON/OFF"
  auto docTgl = [&tr](const std::string& doc, const bool& val)
  { return std::pair(doc, tr(val ? "ON" : "OFF")); };

  // "doc", "ON/OFF/Unset"
  auto docTglOpt = [&tr](const std::string& doc, const std::optional<bool>& val)
  { return std::pair(doc, (val.has_value() ? (val.value() ? tr("ON") : tr("OFF")) : tr("Unset"))); };

  // "Cycle verbose level", "current_level"
  auto docVerbose = [&]()
  {
    return std::pair(
      tr("Verbose level"), this->Internals->VerboseLevelToString(log::getVerboseLevel()));
  };
  ...
  this->addBinding({mod_t::NONE, "B"}, "toggle ui.scalar_bar", "Scene", std::bind(docTgl, tr("Scalar bar"), std::cref(opts.ui.scalar_bar)), f3d::interactor::BindingType::TOGGLE);
  this->addBinding({mod_t::NONE, "P"}, "cycle_blending", "Scene", docBlend, f3d::interactor::BindingType::CYCLIC);
  this->addBinding({mod_t::NONE, "Q"}, "toggle render.effect.ambient_occlusion","Scene", std::bind(docTgl, tr("Ambient occlusion"), std::cref(opts.render.effect.ambient_occlusion)), f3d::interactor::BindingType::TOGGLE);
  this->addBinding({mod_t::NONE, "A"}, "cycle_anti_aliasing","Scene", docAA, f3d::interactor::BindingType::CYCLIC);
  this->addBinding({mod_t::NONE, "T"}, "toggle render.effect.tone_mapping","Scene", std::bind(docTgl, tr("Toggle tone mapping"), std::cref(opts.render.effect.tone_mapping)), f3d::interactor::BindingType::TOGGLE);
  this->addBinding({mod_t::NONE, "E"}, "toggle render.show_edges","Scene", std::bind(docTglOpt, tr("Toggle edges display"), std::cref(opts.render.show_edges)), f3d::interactor::BindingType::TOGGLE);
  this->addBinding({mod_t::NONE, "X"}, "toggle ui.axis","Scene", std::bind(docTgl, tr("Toggle axes display"), std::cref(opts.ui.axis)), f3d::interactor::BindingType::TOGGLE);
  this->addBinding({mod_t::NONE, "G"}, "toggle render.grid.enable","Scene", std::bind(docTgl, tr("Toggle grid display"), std::cref(opts.render.grid.enable)), f3d::interactor::BindingType::TOGGLE);
  this->addBinding({mod_t::SHIFT, "X"}, "toggle render.axes_grid.enable", "Scene", std::bind(docTgl, tr("Toggle axes grid display"), std::cref(opts.render.axes_grid.enable)), f3d::interactor::BindingType::TOGGLE);
#if F3D_MODULE_UI
  this->addBinding({mod_t::NONE, "N"}, "toggle ui.filename","Scene", std::bind(docTgl, tr("Filename"), std::cref(opts.ui.filename)), f3d::interactor::BindingType::TOGGLE);
  this->addBinding({mod_t::NONE, "M"}, "toggle ui.metadata","Scene", std::bind(docTgl, tr("Metadata"), std::cref(opts.ui.metadata)), f3d::interactor::BindingType::TOGGLE);
  this->addBinding({mod_t::SHIFT, "N"}, "toggle ui.hdri_filename","Scene", std::bind(docTgl, tr("HDRI filename"), std::cref(opts.ui.hdri_filename)), f3d::interactor::BindingType::TOGGLE);
  this->addBinding({mod_t::SHIFT, "H"}, "toggle ui.scene_hierarchy","Scene", std::bind(docTgl, tr("Scene hierarchy"), std::cref(opts.ui.scene_hierarchy)), f3d::interactor::BindingType::TOGGLE);
  this->addBinding({mod_t::NONE, "Z"}, "toggle ui.fps","Scene", std::bind(docTgl, tr("FPS Counter"), std::cref(opts.ui.fps)), f3d::interactor::BindingType::TOGGLE);
#endif
#if F3D_MODULE_RAYTRACING
  this->addBinding({mod_t::NONE, "R"}, "toggle render.raytracing.enable","Scene", std::bind(docTgl, tr("Raytracing rendering"), std::cref(opts.render.raytracing.enable)), f3d::interactor::BindingType::TOGGLE);
  this->addBinding({mod_t::NONE, "D"}, "toggle render.raytracing.denoise","Scene", std::bind(docTgl, tr("Denoising when raytracing"), std::cref(opts.render.raytracing.denoise)), f3d::interactor::BindingType::TOGGLE);
#endif
  this->addBinding({mod_t::NONE, "V"}, "toggle_volume_rendering","Scene", std::bind(docTgl, tr("Volume rendering"), std::cref(opts.model.volume.enable)), f3d::interactor::BindingType::TOGGLE);
  this->addBinding({mod_t::NONE, "I"}, "toggle model.volume.inverse","Scene", std::bind(docTgl, tr("Inverse volume opacity"), std::cref(opts.model.volume.inverse)), f3d::interactor::BindingType::TOGGLE);
  this->addBinding({mod_t::CTRL, "N"}, "toggle model.normal_glyphs.enable","Scene", std::bind(docTgl, tr("Normal glyphs"), std::cref(opts.model.normal_glyphs.enable)), f3d::interactor::BindingType::TOGGLE);
  this->addBinding({mod_t::NONE, "O"}, "cycle_point_sprites","Scene", docPS, f3d::interactor::BindingType::CYCLIC);
  this->addBinding({mod_t::NONE, "U"}, "toggle render.background.blur.enable","Scene", std::bind(docTgl, tr("Blur background"), std::cref(opts.render.background.blur.enable)), f3d::interactor::BindingType::TOGGLE);
  this->addBinding({mod_t::NONE, "K"}, "cycle_interactor_style","Scene", std::bind(docStr, tr("Interaction style"), std::cref(opts.interactor.style)), f3d::interactor::BindingType::CYCLIC);
  this->addBinding({mod_t::NONE, "F"}, "toggle render.hdri.ambient","Scene", std::bind(docTgl, tr("HDRI ambient lighting"), std::cref(opts.render.hdri.ambient)), f3d::interactor::BindingType::TOGGLE);
  this->addBinding({mod_t::NONE, "J"}, "toggle render.background.skybox","Scene", std::bind(docTgl, tr("HDRI skybox"), std::cref(opts.render.background.skybox)), f3d::interactor::BindingType::TOGGLE);
  this->addBinding({mod_t::NONE, "L"}, "increase_light_intensity", "Scene", std::bind(docDbl, tr("Increase lights intensity"), std::cref(opts.render.light.intensity)), f3d::interactor::BindingType::NUMERICAL);
  this->addBinding({mod_t::SHIFT, "L"}, "decrease_light_intensity", "Scene", std::bind(docDbl, tr("Decrease lights intensity"), std::cref(opts.render.light.intensity)), f3d::interactor::BindingType::NUMERICAL);
  this->addBinding({mod_t::CTRL, "P"}, "increase_opacity", "Scene", std::bind(docDblOpt, tr("Increase opacity"), std::cref(opts.model.color.opacity)), f3d::interactor::BindingType::NUMERICAL);
  this->addBinding({mod_t::SHIFT, "P"}, "decrease_opacity", "Scene", std::bind(docDblOpt, tr("Decrease opacity"), std::cref(opts.model.color.opacity)), f3d::interactor::BindingType::NUMERICAL);
  this->addBinding({mod_t::SHIFT, "A"}, "toggle render.armature.enable","Scene", std::bind(docTgl, tr("Armature"), std::cref(opts.render.armature.enable)), f3d::interactor::BindingType::TOGGLE);
  this->addBinding({mod_t::ANY, "1"}, "set_camera front", "Camera", std::bind(docStr, tr("Front View camera"), ""));
  this->addBinding({mod_t::ANY, "2"}, "elevation_camera -90", "Camera", std::bind(docStr, tr("Rotate camera down"), ""));
  this->addBinding({mod_t::ANY, "3"}, "set_camera right", "Camera", std::bind(docStr, tr("Right View camera"), ""));
  this->addBinding({mod_t::ANY, "4"}, "roll_camera -90", "Camera", std::bind(docStr, tr("Rotate camera right"), ""));
  this->addBinding({mod_t::ANY, "5"}, "toggle scene.camera.orthographic", "Camera", std::bind(docTglOpt, tr("Orthographic Projection"), std::cref(opts.scene.camera.orthographic)), f3d::interactor::BindingType::TOGGLE);
  this->addBinding({mod_t::ANY, "6"}, "roll_camera 90", "Camera", std::bind(docStr, tr("Rotate camera left"), ""));
  this->addBinding({mod_t::ANY, "7"}, "set_camera top", "Camera", std::bind(docStr, tr("Top View camera"), ""));
  this->addBinding({mod_t::ANY, "8"}, "elevation_camera 90", "Camera", std::bind(docStr, tr("Rotate camera up"), ""));
  this->addBinding({mod_t::ANY, "9"}, "set_camera isometric", "Camera", std::bind(docStr, tr("Isometric View camera"), ""));
  ...
  this->addBinding({mod_t::NONE, "Space"}, "toggle_animation", "Others", std::bind(docStr, tr("Play/Pause animation if any"), ""));
  this->addBinding({mod_t::CTRL_SHIFT, "Space"}, "toggle_animation_backward", "Others", std::bind(docStr, tr("Play/Pause animation backward if any"), ""));
  this->addBinding({mod_t::NONE, "Drop"}, "add_files", "Others", std::bind(docStr, tr("Add files to the scene"), ""), f3d::interactor::BindingType::OTHER, true);
